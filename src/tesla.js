...
function close_charge_port(vid, cb ){
    request( {
        method: 'POST', 
        url: portal + '/vehicles/' + vid + '/command/charge_port_door_close',
        gzip: true,
        headers: http_header
    }, function (error, response, body) { 
        if ((!!error) || (response.statusCode !== 200)) return report(error, response, body, cb);
        try {
            ...
            return report2('charge_port_door_close', body, cb);
        }
    });
}
exports.close_charge_port = close_charge_port;
